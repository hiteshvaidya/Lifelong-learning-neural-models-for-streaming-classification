...
class Network():
    ...
    def backward(self, x, y):
        ...
        with tf.GradientTape() as tape:
            predicted, left_network_loss = self.forward(x)
            current_loss = self.loss(y, predicted) + tau * left_network_loss
...
def custom_KLD(q, p, offset=1e-7): # q is model predictive/approximating distribution, p is target distribution
    ...
    p_ = tf.clip_by_value(p, offset, 1 - offset)
    vec_bce = tf.reduce_mean(p_ * (tf.math.log(p_) - tf.math.log(q_)))
    return tf.reduce_mean(vec_bce)/p.shape[0]
...
trainX = pd.read_csv('mnist_clean/trainX.tsv', sep="\t", header=None, index_col=False, dtype=np.float32).to_numpy()
trainY = pd.read_csv('mnist_clean/trainY.tsv', sep="\t", header=None, index_col=False, dtype=np.float32).to_numpy()
testX = pd.read_csv('mnist_clean/testX.tsv', sep="\t", header=None, index_col=False, dtype=np.float32).to_numpy()
testY = pd.read_csv('mnist_clean/testY.tsv', sep="\t", header=None, index_col=False, dtype=np.float32).to_numpy()
validX = pd.read_csv('mnist_clean/validX.tsv', sep="\t", header=None, index_col=False, dtype=np.float32).to_numpy()
validY = pd.read_csv('mnist_clean/validY.tsv', sep="\t", header=None, index_col=False, dtype=np.float32).to_numpy()
...
#             loss += (custom_bce(output, temp) + tau*autoencoder_loss) * batch.shape[0]
            loss += (net.loss(temp, output) + tau * autoencoder_loss) * \
                    batch.shape[0]
...
#             valid_loss += (custom_bce(output, temp) + tau*autoencoder_loss) * batch.shape[0]
            valid_loss += (net.loss(temp, output) + tau * autoencoder_loss) * \
                          batch.shape[0]
            loss_at += autoencoder_loss * batch.shape[0]
            output = output.numpy().reshape(-1)
            output[output >= 0.5] = 1
...
#         test_loss += (custom_bce(output, temp) + autoencoder_loss) * batch.shape[0]
        test_loss += (net.loss(temp, output) + tau * autoencoder_loss) * \
                     batch.shape[0]
        loss_at += autoencoder_loss * batch.shape[0]
        output = output.numpy().reshape(-1)
        output[output >= 0.5] = 1
...
pkl.dump(data_dict, open('results/dpn_data_dict_tau=0.pkl', 'wb'))
