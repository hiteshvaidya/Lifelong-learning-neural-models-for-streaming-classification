...
class Network():
    ...
    def forward(self, x, tau=0.0):
        ...
        at_loss = custom_bce(Z1, Z1_hat) + custom_bce(x, Z0_hat)
        return Z0_hat, at_loss

    def loss(self, x, Z1, Z0, choice='log'):
        '''
        y_pred - Tensor of shape (batch_size, size_output)
        y_true - Tensor of shape (batch_size, size_output)
        '''
        y_true_tf = tf.cast(tf.reshape(Z1, (-1, 1)), dtype=tf.float32)
        y_pred_tf = tf.cast(tf.reshape(Z0, (-1, 1)), dtype=tf.float32)
        if choice == 'log':
            return tf.reduce_mean(tf.compat.v1.losses.log_loss(Z0,
                                                               x)) + tf.reduce_mean(tf.compat.v1.losses.log_loss(Z1, x))
        elif choice == 'bce':
...
def custom_KLD(p, q,
               offset=1e-7):  # q is model predictive/approximating distribution, p is target distribution
    '''
...
def valid_metrics():
    ...
    loss_at = 0
    size = 0
    batches = batch_loader(validY, batch_size)
    for batch in batches:
        Z0_hat, batch_loss = net.forward(validX[batch])
        size += batch.shape[0]
        loss_at += batch_loss * batch.shape[0]
    valid_losses.append(loss_at / size)
...
# test the model
test_loss = 0
batches = batch_loader(testX, batch_size)
for batch in batches:
    Z0_hat, batch_loss = net.forward(testX[batch])
    test_loss += batch_loss * batch.shape[0]
test_losses.append(test_loss / testX.shape[0])
...
def plot_fig():
    ...
    for row in range(3):
        ...
        print('Mean of input =', mean2.numpy())
